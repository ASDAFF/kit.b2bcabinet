...
if(Loader::includeModule('kit.b2bcabinet') && $idUser > 0)
{
    ...
    <div class="wait_pay">
        <div class="widget_content widget_payment_waiting">
            <h5><?=Loc::getMessage('GD_KIT_CABINET_BUYORDER_SUM')?> </h5>
            <h4><b><?=$order->getPrice() ?></b></h4>
        </div>
        <div class="widget_content widget_links_btns widget-payment_waiting-content">
            <div>
                <span class="payment_waiting_text"><?=Loc::getMessage('GD_KIT_CABINET_BUYORDER_DATE')?></span>
                <span><?=$order->getDate()->format("d.m.Y H:i:s")?></span>
            </div>
            <div>
                <span class="payment_waiting_text"><?=Loc::getMessage('GD_KIT_CABINET_BUYORDER_PERSON_TYPE')?></span>
