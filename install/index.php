...
class kit_b2bcabinet extends CModule
{
    const MODULE_ID = 'kit.b2bcabinet';
    const TEMPLATE_NAME = 'b2bcabinet';
    var $MODULE_ID = 'kit.b2bcabinet';
    var $MODULE_VERSION;
    ...
    function __construct()
    {
        ...
        $this->MODULE_NAME = GetMessage('kit.b2bcabinet_MODULE_NAME');
        $this->MODULE_DESCRIPTION = GetMessage('kit.b2bcabinet_MODULE_DESC');
        $this->PARTNER_NAME = GetMessage('kit.b2bcabinet_PARTNER_NAME');
        $this->PARTNER_URI = GetMessage('kit.b2bcabinet_PARTNER_URI');
    }
    ...
    {
        EventManager::getInstance()->registerEventHandler('main', 'OnBuildGlobalMenu', self::MODULE_ID, '\Kit\B2bCabinet\EventHandlers', 'onBuildGlobalMenuHandler');
        EventManager::getInstance()->registerEventHandler('main', 'OnPageStart', self::MODULE_ID, '\Kit\B2bCabinet\EventHandlers', 'onPageStart');
        return true;
    }
    ...
    {
        EventManager::getInstance()->unRegisterEventHandler('main', 'OnBuildGlobalMenu', self::MODULE_ID, '\Kit\B2bCabinet\EventHandlers', 'OnBuildGlobalMenuHandler');
        EventManager::getInstance()->unRegisterEventHandler('main', 'OnPageStart', self::MODULE_ID, '\Kit\B2bCabinet\EventHandlers', 'onPageStart');
        return true;
    }
    ...
    function UnInstallFiles()
    {
        ...
        DeleteDirFiles($_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/' . self::MODULE_ID . '/install/wizards/kit/b2bcabinet/site/templates/b2bcabinet', $_SERVER['DOCUMENT_ROOT'] . '/local/templates/b2bcabinet');
        $_1073068217 = $_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/' . self::MODULE_ID . '/install/wizards/kit/b2bcabinet/site/public/ru/';
        ...
        if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/local/components/kit/b2bcabinet.catalog.smart.filter')) {
            $_39244912 = array();
            $_39244912 = scandir($_SERVER['DOCUMENT_ROOT'] . '/local/components/kit/');
            if (!empty($_39244912)) {
                foreach ($_39244912 as $_113930043) {
                    if ($_113930043 == '.' || $_113930043 == '..') {
                        continue;
                    }
                    if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/local/components/kit/' . $_113930043)) {
                        DeleteDirFilesEx('/local/components/kit/' . $_113930043);
                    }
                }
            }
            DeleteDirFiles($_1073068217 . 'root/local/components/kit', $_SERVER['DOCUMENT_ROOT'] . '/local/components/kit');
        }
        return true;
    }
