	...
	function getMacro($arrSites)
	{
		...
		if($siteDirs)
		{
			foreach($siteDirs as $sid => $siteDir)
			{
				...
				//2
				$text = file_get_contents($_SERVER['DOCUMENT_ROOT'].'/local/templates/b2bcabinet/components/bitrix/news/kit_origami_shops/detail.php');
				preg_match_all("#WEB_FORM_ID(.*)\n#", $text, $matches);
				if($matches[0])
				{
					foreach($matches[0] as $match)
	...
	function backupModuleFiles($str)
	{
		...
		if($files)
		{
			foreach($files as $file)
			{
				...
				CreateBakFile($_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/kit.b2bcabinet/'.$path);
			}
		}
	}
}
...
if (!function_exists("backupTemplateFiles"))
{
	...
	{
		$dir = __DIR__.'/install/wizards/kit/b2bcabinet/site/templates/b2bcabinet';
...
if (!function_exists("backupPublicFiles"))
{
	...
	{
		$dir = __DIR__.'/install/wizards/kit/b2bcabinet';
		$files = readDirectory($dir);
		if($files && $arrSites)
		{
			foreach($arrSites as $sid => $path_site)
