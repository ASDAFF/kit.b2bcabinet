            ...
            if($arDay["OPEN_TIME"] == "CUSTOM")
            {
                foreach ($arDay["CUSTOM_TIME"] as $ar)
                {
                    ...
                    if($presMF || $presMT)
                    {
                        ...
                        $minute_from = SotbitWizardStrToTime(($presMF ? $ar["MINUTE_FROM"] : "00:00"));
                        $minute_till = SotbitWizardStrToTime(($presMT ? $ar["MINUTE_TILL"] : "23:59"));
...
function SotbitWizardStrToTime($t)
{
    $a = explode(":", $t);
