                ...
                foreach ($res as $k => $r)
                {
                    ...
                    if($haveOffers && $i == 0) {
                        if (Bitrix\Main\Loader::includeModule( "sotbit.privateprice" ) && Bitrix\Main\Loader::includeModule( "currency" )) {
                            $privatePrice = SotbitPrivatePriceMain::setPlaceholder($item['OFFERS'][$i]['ID'], CCurrencyLang::CurrencyFormat((!empty($minPrice[$k]) ? $minPrice[$k] : $price['DEF']), CCurrency::GetBaseCurrency()));
                            $privatePriceValue = SotbitPrivatePriceMain::setPlaceholderValue($item['OFFERS'][$i]['ID'], CCurrencyLang::CurrencyFormat((!empty($minPrice[$k]) ? $minPrice[$k] : $price['DEF']), CCurrency::GetBaseCurrency()));
                        }
                        if (Bitrix\Main\Loader::includeModule( "sotbit.privateprice" ) && isset($privatePrice) && isset($privatePriceValue)) {
                        ...
                        if (Bitrix\Main\Loader::includeModule( "sotbit.privateprice" ) && Bitrix\Main\Loader::includeModule( "currency" )) {
                            $privatePrice = SotbitPrivatePriceMain::setPlaceholder($item['ID'], (isset($price['FORMAT']) && !empty($price['FORMAT']) ? $price['FORMAT'] : ''));
                            $privatePriceValue = SotbitPrivatePriceMain::setPlaceholderValue($item['ID'], (isset($price['FORMAT']) && !empty($price['FORMAT']) ? $price['FORMAT'] : ''));
                        }
